...
// Cloud sync server URL - defaults to localhost (the local Go backend).
// Set BUSINESSOS_SYNC_URL env variable to enable cloud sync with a remote server.
const SYNC_SERVER_URL =
  process.env.BUSINESSOS_SYNC_URL || "http://localhost:8000";
...
export function initializeDatabaseSystem(): void {
  ...
  // Initialize sync engine pointing to the local backend by default.
  // Set BUSINESSOS_SYNC_URL to enable optional cloud sync.
  ...
  console.log(`Sync engine configured with server: ${SYNC_SERVER_URL}`);
}
...
export function setupDatabaseHandlers(): void {
  ...
  ipcMain.handle(
    "db:getAll",
    async (_, table: string, where?: Record<string, any>) => {
      try {
  ...
  ipcMain.handle(
    "db:create",
    async (_, table: string, data: Record<string, any>) => {
      try {
  ...
  ipcMain.handle(
    "db:update",
    async (_, table: string, id: string, data: Record<string, any>) => {
      try {
        const updates = {
          ...data,
          updated_at: new Date().toISOString(),
  ...
  ipcMain.handle("db:clients:getWithDeals", async (_, clientId: string) => {
    try {
      ...
      return {
        success: true,
        data: { ...client, contacts, deals, interactions },
      };
    } catch (error) {
      return { success: false, error: String(error) };
    }
  ...
  ipcMain.handle(
    "db:settings:upsert",
    async (_, userId: string, settings: Record<string, any>) => {
      try {
