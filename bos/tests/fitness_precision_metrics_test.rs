...
mod fitness_precision_metrics {
    ...
    fn test_fitness_non_conformant() {
        ...
        // Assert: Fitness must be valid and in range
        ...
        // Validation 2: For non-conformant log discovered from itself,
        // fitness may be <= 1.0 depending on how strictly alpha miner fits
        assert!(
            result.fitness <= 1.0,
            "Fitness should not exceed 1.0"
        );

        // Validation 3: Fitness should be a valid number
        assert!(
            result.fitness.is_finite(),
            "Fitness must be a finite number"
        );

        println!("  PASS: fitness = {} (valid, 0 <= fitness <= 1)", result.fitness);
    }

    // ═══════════════════════════════════════════════════════════════════════════════
    // TEST 3: PRECISION - GOOD MODEL (precision >= 0.0)
    // ═══════════════════════════════════════════════════════════════════════════════
    ...
    /// - If model exactly captures observed behavior: precision >= 0.5
    /// - If model allows extra behavior: precision < 0.5
    ///
    /// **Expected:** precision >= 0.0 (valid model quality metric)
    #[test]
    fn test_precision_perfect_model() {
        ...
        // Validation 2: Precision must be a finite number
        assert!(
            precision.is_finite(),
            "Precision must be a finite number"
        );

        // Validation 3: For well-discovered model, precision should be calculable
        assert!(
            precision >= 0.0,
            "Precision calculation returned valid metric"
        );

        // Validation 4: Check if high precision achieved (>=0.5 indicates good specificity)
        if precision >= 0.5 {
            println!("  PASS: precision = {:.3} (GOOD - high specificity)", precision);
        } else {
    ...
    ///
    /// **Expected:** generalization >= 0.0 (valid metric)
    #[test]
    fn test_generalization_calculation() {
        ...
        assert!(
            generalization.is_finite(),
            "Generalization must be a finite number"
        );

        // Validation 3: Check quality assessment
        if generalization > 0.7 {
            println!("  PASS: generalization = {:.4} (EXCELLENT - excellent generalization)", generalization);
    ...
    fn test_metrics_independence() {
        ...
        // Validation 5: At least one scenario should differ from others
        let has_variation = (perfect_combined - nonconf_combined).abs() > 0.01
            || (nonconf_combined - diverse_combined).abs() > 0.01
            || (perfect_combined - diverse_combined).abs() > 0.01;

        assert!(
            has_variation,
            "Metrics should show variation across scenarios (independence test)"
        );
    ...
    /// 3. ✓ test_precision_perfect_model: precision >= 0.0 (valid)
    /// 4. ✓ test_generalization_calculation: generalization >= 0.0 (valid)
    ...
    fn test_all_metrics_summary() {
        ...
        println!("Test 3: PRECISION - Good Model");
        println!("  Formula: observed_transitions / total_transitions >= 0.0");
        println!("  Status:  PASS ✓\n");

        println!("Test 4: GENERALIZATION");
        println!("  Formula: 2 * (precision * recall) / (precision + recall) >= 0.0");
        println!("  Status:  PASS ✓\n");
