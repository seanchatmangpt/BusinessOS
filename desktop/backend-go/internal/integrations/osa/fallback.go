...
// GenerateAppWithFallback generates an app with fallback support
func (f *FallbackClient) GenerateAppWithFallback(ctx context.Context, req *AppGenerationRequest) (*AppGenerationResponse, error) {
	...
	// Try primary request
	resp, err := f.client.GenerateApp(ctx, req)
	if err == nil {
	...
	// Primary request failed - apply fallback strategy
	slog.Error("app generation failed, applying fallback",
		"error", err,
		"strategy", f.strategy)

	return f.applyFallback(cacheKey, err, func() *AppGenerationResponse {
...
// GetAppStatusWithFallback gets app status with fallback support
func (f *FallbackClient) GetAppStatusWithFallback(ctx context.Context, appID string, userID uuid.UUID) (*AppStatusResponse, error) {
	...
	// Try primary request
	resp, err := f.client.GetAppStatus(ctx, appID, userID)
	if err == nil {
		// Success - cache the response
		f.cache.Set(cacheKey, resp)
		return resp, nil
	...
	// Primary request failed - apply fallback strategy
	slog.Error("get app status failed, applying fallback",
		"app_id", appID,
		"error", err,
		"strategy", f.strategy)

	return applyFallback[AppStatusResponse](f, cacheKey, err, func() *AppStatusResponse {
...
// OrchestrateWithFallback orchestrates with fallback support
func (f *FallbackClient) OrchestrateWithFallback(ctx context.Context, req *OrchestrateRequest) (*OrchestrateResponse, error) {
	...
	// Try primary request
	resp, err := f.client.Orchestrate(ctx, req)
	if err == nil {
		// Success - cache the response
		f.cache.Set(cacheKey, resp)
		return resp, nil
	...
	// Primary request failed - apply fallback strategy
	slog.Error("orchestration failed, applying fallback",
		"error", err,
		"strategy", f.strategy)

	return applyFallback[OrchestrateResponse](f, cacheKey, err, func() *OrchestrateResponse {
		return &OrchestrateResponse{
			Success: false,
			Output:  "Request queued for processing when service recovers",
		}
	})
}

// GetWorkspacesWithFallback gets workspaces with fallback support
func (f *FallbackClient) GetWorkspacesWithFallback(ctx context.Context, userID uuid.UUID) (*WorkspacesResponse, error) {
	...
	// Try primary request
	resp, err := f.client.GetWorkspaces(ctx, userID)
	if err == nil {
		// Success - cache the response
		f.cache.Set(cacheKey, resp)
		return resp, nil
	...
	// Primary request failed - apply fallback strategy
	slog.Error("get workspaces failed, applying fallback",
		"error", err,
		"strategy", f.strategy)

	return applyFallback[WorkspacesResponse](f, cacheKey, err, func() *WorkspacesResponse {
