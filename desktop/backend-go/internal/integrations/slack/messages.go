...
func (s *MessageService) SyncMessages(ctx context.Context, userID, channelID string, limit int) (*SyncMessagesResult, error) {
	log.Printf("Slack message sync starting for user %s, channel %s", userID, channelID)
	...
	for _, msg := range messages {
		if err := s.saveMessage(ctx, userID, channelID, msg); err != nil {
			log.Printf("Failed to save message %s: %v", msg.TS, err)
	...
	log.Printf("Slack message sync complete: synced %d/%d messages",
		result.SyncedMessages, result.TotalMessages)
...
func (s *MessageService) SendMessage(ctx context.Context, userID, channelID, text string) error {
	...
	log.Printf("Sent message to channel %s, ts: %s", slackChannelID, result.TS)
	return nil
}
