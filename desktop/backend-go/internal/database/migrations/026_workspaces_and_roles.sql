...
CREATE INDEX idx_workspace_roles_hierarchy ON workspace_roles(workspace_id, hierarchy_level);
...
CREATE INDEX idx_workspace_memories_type ON workspace_memories(workspace_id, memory_type);
CREATE INDEX idx_workspace_memories_visibility ON workspace_memories(workspace_id, visibility);
CREATE INDEX idx_workspace_memories_embedding ON workspace_memories USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
...
-- Note: This table may already exist from previous migrations
...
-- Add notification_level column if not exists
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
...
-- Add created_at/updated_at if not exists
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
...
CREATE INDEX idx_role_permissions_role ON role_permissions(workspace_id, role);
