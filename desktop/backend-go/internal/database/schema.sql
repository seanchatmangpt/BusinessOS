...
CREATE TABLE osa_module_instances (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_osa_apps_workspace ON osa_module_instances(workspace_id);
CREATE INDEX idx_osa_apps_module ON osa_module_instances(module_id);
CREATE INDEX idx_osa_apps_status ON osa_module_instances(status);
CREATE INDEX idx_osa_apps_workflow ON osa_module_instances(osa_workflow_id);
CREATE INDEX idx_osa_apps_deployment_port ON osa_module_instances(deployment_port);
CREATE INDEX idx_osa_apps_category ON osa_module_instances(category);
CREATE INDEX idx_osa_apps_workspace_category ON osa_module_instances(workspace_id, category);
-- Sandbox indexes (Migration 091)
CREATE INDEX idx_osa_apps_container_id ON osa_module_instances(container_id) WHERE container_id IS NOT NULL;
CREATE INDEX idx_osa_apps_sandbox_status ON osa_module_instances(sandbox_status) WHERE sandbox_status != 'none';
CREATE INDEX idx_osa_apps_sandbox_port ON osa_module_instances(sandbox_port) WHERE sandbox_port IS NOT NULL;
CREATE INDEX idx_osa_apps_health_status ON osa_module_instances(health_status) WHERE health_status != 'unknown';
...
CREATE INDEX idx_sandbox_events_module_instance ON sandbox_events(module_instance_id);
CREATE INDEX idx_sandbox_events_type ON sandbox_events(event_type);
CREATE INDEX idx_sandbox_events_created ON sandbox_events(created_at DESC);
...
CREATE INDEX idx_osa_exec_app ON osa_execution_history(module_instance_id);
...
CREATE INDEX idx_osa_build_app ON osa_build_events(module_instance_id);
...
CREATE INDEX idx_osa_webhooks_app ON osa_webhooks(module_instance_id);
...
CREATE INDEX idx_osa_workflows_app ON osa_workflows(module_instance_id);
...
CREATE INDEX idx_osa_files_workflow ON osa_module_files(workflow_id);
CREATE INDEX idx_osa_files_app ON osa_module_files(module_instance_id);
CREATE INDEX idx_osa_files_type ON osa_module_files(file_type);
CREATE INDEX idx_osa_files_language ON osa_module_files(language);
CREATE INDEX idx_osa_files_status ON osa_module_files(installation_status);
CREATE INDEX idx_osa_files_hash ON osa_module_files(content_hash);
CREATE INDEX idx_osa_files_path ON osa_module_files(file_path);
CREATE INDEX idx_osa_files_name ON osa_module_files(file_name);
CREATE INDEX idx_osa_files_latest ON osa_module_files(is_latest) WHERE is_latest = true;

CREATE TABLE osa_file_versions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE osa_installation_log (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workflow_id UUID NOT NULL REFERENCES osa_workflows(id) ON DELETE CASCADE,
...
CREATE TABLE user_generated_apps (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workspace_id UUID NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
...
CREATE INDEX idx_user_generated_apps_module_instance ON user_generated_apps(module_instance_id);
...
CREATE TABLE app_generation_queue (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workspace_id UUID NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
...
CREATE INDEX idx_module_versions_stable ON custom_module_versions(module_id, is_stable) WHERE is_stable = TRUE;
...
CREATE INDEX idx_module_installations_enabled ON module_installations(workspace_id, is_enabled) WHERE is_enabled = TRUE;
...
CREATE INDEX idx_module_shares_user ON module_shares(shared_with_user_id) WHERE shared_with_user_id IS NOT NULL;
CREATE INDEX idx_module_shares_workspace ON module_shares(shared_with_workspace_id) WHERE shared_with_workspace_id IS NOT NULL;
CREATE INDEX idx_module_shares_email ON module_shares(shared_with_email) WHERE shared_with_email IS NOT NULL;
...
CREATE UNIQUE INDEX idx_osa_prompt_name_scope_unique ON osa_prompt_templates(
    name,
    scope,
