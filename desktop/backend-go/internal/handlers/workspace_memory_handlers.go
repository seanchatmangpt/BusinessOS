...
func (h *WorkspaceMemoryHandlers) CreateMemory(c *gin.Context) {
	...
	if req.Visibility != "workspace" && req.Visibility != "private" && req.Visibility != "shared" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid visibility. Must be workspace, private, or shared"})
		return
	}
	...
	if !isValidType {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid memory_type. Must be one of: general, decision, pattern, context, learning, preference"})
		return
	}
	...
	if createErr != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to create memory: " + createErr.Error()})
		return
	}
...
func (h *WorkspaceMemoryHandlers) ListWorkspaceMemories(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch memories: " + err.Error()})
		return
	}
...
func (h *WorkspaceMemoryHandlers) ShareMemory(c *gin.Context) {
	...
	if memoryWorkspaceID != workspaceID {
		c.JSON(http.StatusForbidden, gin.H{"error": "Memory does not belong to this workspace"})
		return
	}
...
func (h *WorkspaceMemoryHandlers) UnshareMemory(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to unshare memory: " + err.Error()})
		return
	}
...
func (h *WorkspaceMemoryHandlers) DeleteMemory(c *gin.Context) {
	...
	if visibility != "workspace" {
		if ownerUserID == nil || *ownerUserID != user.ID {
			c.JSON(http.StatusForbidden, gin.H{"error": "Only the owner can delete this memory"})
			return
		}
	} else {
		...
		if err != nil {
			c.JSON(http.StatusForbidden, gin.H{"error": "Not authorized to delete workspace memories"})
			return
		}

		if role != "owner" && role != "admin" {
			c.JSON(http.StatusForbidden, gin.H{"error": "Only workspace owners and admins can delete workspace memories"})
			return
		}
	}
