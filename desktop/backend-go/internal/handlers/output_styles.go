...
func (h *Handlers) ListOutputStyles(c *gin.Context) {
	...
	`)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to list output styles"})
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to scan output style"})
			return
		}
...
func (h *Handlers) UpsertUserOutputPreference(c *gin.Context) {
	...
	if req.DefaultStyleID != nil && *req.DefaultStyleID != "" {
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid default_style_id"})
			return
		}
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "Unknown default_style_name"})
			return
		}
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid style_overrides"})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to save output preference"})
		return
	}
