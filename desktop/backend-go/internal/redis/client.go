...
func Connect(ctx context.Context, cfg *Config) error {
	...
	once.Do(func() {
		...
		if cfg.Password != "" {
			...
			log.Printf("Redis: Password authentication enabled")
		}
		...
		if cfg.TLSEnabled {
			...
			if cfg.TLSInsecure {
				...
				log.Printf("WARNING: Redis TLS enabled with InsecureSkipVerify=true (dev only)")
			} else {
				log.Printf("Redis: TLS enabled with certificate validation")
			}
		...
		log.Printf("Redis connected: %s (pool_size=%d, protocol=%s)", safeURL, cfg.PoolSize, tlsStatus)
	})
