...
// skipIfNoDB skips the test when no database connection is available.
func skipIfNoDB(t *testing.T, service *ModelHistoryService) {
	t.Helper()
	if service.pool == nil {
		t.Skip("skipping: requires database connection (set TEST_DB_URL)")
	}
}
