...
func (s *ModelHistoryService) CreateVersion(
	...
	// Nil-pool guard: return constructed version for unit testing without a database.
	if s.pool == nil {
		major, minor, patch := s.determineVersion(changeType, nil)
		...
		return &ProcessModelVersion{
			...
			EdgesCount:      metrics.EdgesCount,
			Variants:        metrics.Variants,
			Fitness:         metrics.Fitness,
			AverageDuration: metrics.AverageDuration,
			CoveredTraces:   metrics.CoveredTraces,
			ChangeType:      changeType,
			NodesAdded:      changeSummary.NodesAdded,
			NodesRemoved:    changeSummary.NodesRemoved,
			EdgesAdded:      changeSummary.EdgesAdded,
			EdgesRemoved:    changeSummary.EdgesRemoved,
	...
	_ = identifyBreakingChanges(changeType, changeSummary)
...
func (s *ModelHistoryService) GetVersion(
	...
	if s.pool == nil {
		return nil, fmt.Errorf("get version: database not available")
	}
...
func (s *ModelHistoryService) GetVersionHistory(
	...
	if s.pool == nil {
		return nil, 0, fmt.Errorf("list versions: database not available")
	}
...
func (s *ModelHistoryService) ReleaseVersion(
	...
	if s.pool == nil {
		return fmt.Errorf("release version: database not available")
	}
