...
func TestSessionCache_Set(t *testing.T) {
	...
	cache := NewSessionCache(client, cfg)
...
func TestSessionCache_MultipleUsers(t *testing.T) {
	...
	cache.Set(ctx, "user1-token1", user1)
	cache.Set(ctx, "user1-token2", user1)
	...
	cache.Set(ctx, "user2-token1", user2)
	cache.Set(ctx, "user2-token2", user2)
