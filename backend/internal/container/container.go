...
func getInitScriptPath() string {
	...
	if path := os.Getenv("BUSINESSOS_INIT_SCRIPT"); path != "" {
		if absPath, err := filepath.Abs(path); err == nil {
			if _, err := os.Stat(absPath); err == nil {
				log.Printf("[Container] Using init script from env: %s", absPath)
				return absPath
			}
		}
	}
	...
	if absPath, err := filepath.Abs(relativePath); err == nil {
		if _, err := os.Stat(absPath); err == nil {
			log.Printf("[Container] Found init script at: %s", absPath)
			return absPath
		}
	}
	...
	log.Printf("[Container] Warning: Init script not found, using fallback: %s", fallback)
	return fallback
}
