...
VALUES ($1, $2, $3, $4, $5, $6)
RETURNING id, workspace_id, email, role, invited_by, token, status, expires_at, accepted_at, created_at, updated_at
`
...
const getUserDefaultWorkspace = `-- name: GetUserDefaultWorkspace :one
SELECT
...
type GetUserDefaultWorkspaceRow struct {
	ID           pgtype.UUID        `json:"id"`
	Name         string             `json:"name"`
	Slug         string             `json:"slug"`
	Description  *string            `json:"description"`
...
func (q *Queries) GetUserDefaultWorkspace(ctx context.Context, userID string) (GetUserDefaultWorkspaceRow, error) {
	row := q.db.QueryRow(ctx, getUserDefaultWorkspace, userID)
	var i GetUserDefaultWorkspaceRow
	err := row.Scan(
		&i.ID,
		&i.Name,
		&i.Slug,
...
SELECT id, workspace_id, email, role, invited_by, token, status, expires_at, accepted_at, created_at, updated_at FROM workspace_invites WHERE id = $1 AND workspace_id = $2
`
...
SELECT id, workspace_id, email, role, invited_by, token, status, expires_at, accepted_at, created_at, updated_at FROM workspace_invites
