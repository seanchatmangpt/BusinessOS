...
// AnalyzeContent handles data analysis using the Analyst agent
...
// ExtractTasks extracts actionable tasks from content using Task agent
func (h *Handlers) ExtractTasks(c *gin.Context) {
	...
	// Use Task agent for task extraction
	registry := agents.NewAgentRegistry(h.pool, h.cfg, h.embeddingService, h.promptPersonalizer)
...
// CreatePlan creates a strategic plan using the Project agent
