...
func (h *Handlers) SendMessageV2(c *gin.Context) {
	...
	// Determine model - use provider-aware default instead of generic DefaultModel
	...
	registry := agents.NewAgentRegistry(h.pool, h.cfg, h.embeddingService, h.promptPersonalizer)

	// useCOT is kept in the request struct for API compatibility but COT execution is not used
	_ = req.UseCOT
	...
	// Determine agent type from request field or focus mode
	var agentType agents.AgentType
	var focusSystemPrompt string
	var searchContextText string
	var searchResultCount int

	// Map agent_type request field to agent type
	if req.AgentType != nil && *req.AgentType != "" {
		agentType = agents.AgentTypeFromString(*req.AgentType)
		log.Printf("[ChatV2] Agent type from request: %s", agentType)
	} else {
		agentType = agents.AgentTypeOrchestrator
	}

	// Focus mode can override agent type and apply focus-specific settings
	if req.FocusMode != nil && *req.FocusMode != "" {
		...
		// Map focus mode to agent type (overrides agent_type field)
		focusAgentType := agents.GetAgentForFocusMode(*req.FocusMode)
		if focusAgentType != agents.AgentTypeOrchestrator {
			agentType = focusAgentType
			log.Printf("[ChatV2] FocusMode overriding agent type to: %v", agentType)
		}
	}
	...
	log.Printf("[ChatV2] Profile injection check: user_id=%s", user.ID)
	profileContextStr := ""
	onboardingService := services.NewOnboardingService(h.pool, nil, nil, nil) // AI service, Gmail, and OSA sync not needed for GetUserProfile
	userProfile, err := onboardingService.GetUserProfile(ctx, user.ID)
	if err == nil && userProfile != nil {
		profileContextStr = services.BuildProfilePrefix(userProfile)
		agent.SetProfileContext(profileContextStr)
		log.Printf("[ChatV2] Injected user profile context (%d chars): business=%s, team=%s, role=%s, challenge=%s",
			len(profileContextStr), userProfile.BusinessType, userProfile.TeamSize, userProfile.OwnerRole, userProfile.MainChallenge)
	} else {
		if err != nil {
			log.Printf("[ChatV2] No profile found for user (expected if onboarding not completed): %v", err)
		}
	}
	...
	c.Header("X-Intent-Category", "direct")
...
- [Source Title 2](url2)`, searchContextText, chatMessages[i].Content)
	...
	slog.Debug("ChatV2: Starting stream", "agent", agentType)
...
func (h *Handlers) handleSlashCommandV2(c *gin.Context, user *middleware.BetterAuthUser, req SendMessageRequest) {
	...
	// Inject user onboarding profile context for personalization
	log.Printf("[ChatV2-Slash] Profile injection check: user_id=%s", user.ID)
	...
	if err == nil && userProf != nil {
		...
		log.Printf("[ChatV2-Slash] Injected user profile context (%d chars)", len(profileCtx))
	}
