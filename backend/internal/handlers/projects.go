...
func (h *Handlers) ListProjects(c *gin.Context) {
	...
	// Generate cache key using ProjectListKey helper
	if h.queryCache != nil {
		page := 1 // Add page parameter support in the future
		cacheKey := h.queryCache.ProjectListKey(user.ID, page, filters)

		// Try to get from cache
		var cachedProjects []map[string]interface{}
	...
	projects, err := h.fetchProjectsFromDB(c, user.ID, filters)
	if err != nil {
...
func (h *Handlers) fetchProjectsFromDB(c *gin.Context, userID string, filters map[string]string) (interface{}, error) {
...
func (h *Handlers) CreateProject(c *gin.Context) {
	...
	if h.queryCache != nil {
		// Invalidate all project list caches for this user
		cachePattern := fmt.Sprintf("projects:%s:*", user.ID)
		go h.invalidateProjectsCachePattern(c.Request.Context(), h.queryCache, cachePattern)
	}
...
// This is a helper function for cache invalidation after mutations
func (h *Handlers) invalidateProjectsCachePattern(ctx context.Context, queryCache *cache.QueryCache, pattern string) {
	if queryCache == nil {
		return
	}

	// Use Redis SCAN to find matching keys and delete them
	// This is a background operation, so errors are logged but not critical
	// The cache pattern should be "projects:{userID}:*" to invalidate all project lists for a user
