...
export const categoryColorMap: Record<string, { fg: string; bg: string; text: string }> = {
	finance: { fg: '#10b981', bg: 'rgba(16, 185, 129, 0.1)', text: 'text-green-400' },
	communication: { fg: '#3b82f6', bg: 'rgba(59, 130, 246, 0.1)', text: 'text-blue-400' },
	productivity: { fg: '#a855f7', bg: 'rgba(168, 85, 247, 0.1)', text: 'text-purple-400' },
	analytics: { fg: '#f97316', bg: 'rgba(249, 115, 22, 0.1)', text: 'text-orange-400' },
	ecommerce: { fg: '#ec4899', bg: 'rgba(236, 72, 153, 0.1)', text: 'text-pink-400' },
	crm: { fg: '#06b6d4', bg: 'rgba(6, 182, 212, 0.1)', text: 'text-cyan-400' },
	hr: { fg: '#6366f1', bg: 'rgba(99, 102, 241, 0.1)', text: 'text-indigo-400' },
	inventory: { fg: '#f59e0b', bg: 'rgba(245, 158, 11, 0.1)', text: 'text-amber-400' },
	marketing: { fg: '#f43f5e', bg: 'rgba(244, 63, 94, 0.1)', text: 'text-rose-400' },
	project: { fg: '#14b8a6', bg: 'rgba(20, 184, 166, 0.1)', text: 'text-teal-400' },
	general: { fg: '#6b7280', bg: 'rgba(107, 114, 128, 0.1)', text: 'text-gray-400' }
};
...
export function getCategoryColors(category: string): { fg: string; bg: string; text: string } {
...
function createDeployedAppsStore() {
	...
	async function fetchDeployedApps(): Promise<DeployedApp[]> {
		...
		try {
			const response = await fetch('http://localhost:8001/api/osa/apps/deployed', {
				credentials: 'include',
			});
			...
			update(state => {
				...
				for (const app of apps) {
					if (app.status === 'running') {
						...
						const iconName = app.metadata?.icon || getCategoryIconName(category);
