...
export async function initializeWorkspaces(): Promise<void> {
  ...
  try {
    ...
    if (!current && allWorkspaces.length > 0) {
      console.log(
        `[Workspaces] Auto-selecting first workspace: ${allWorkspaces[0].name} (${allWorkspaces[0].id})`,
      );
      ...
      if (import.meta.env.DEV) {
        console.log(
          "[Workspaces] No workspaces found in dev mode, loading mock data",
        );
...
export async function loadSavedWorkspace(): Promise<void> {
  ...
  if (savedId) {
    try {
      ...
      console.warn(
        "[Workspaces] Failed to load saved workspace, loading all workspaces",
      );
