...
<div class="h-full flex flex-col bg-gray-50/50 dark:bg-[#1c1c1e]">
	<!-- Header -->
	<div class="px-6 py-4 bg-white dark:bg-[#1c1c1e] border-b border-gray-200 dark:border-white/10">
		<div class="flex items-center justify-between mb-4">
			<div>
				<h1 class="text-xl font-semibold text-gray-900 dark:text-white">Projects</h1>
				<p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Manage your work and track progress</p>
			</div>
	...
	<div class="px-6 py-3 bg-white dark:bg-[#1c1c1e] border-b border-gray-200 dark:border-white/10 flex items-center gap-3 flex-wrap">
		<!-- Search -->
		<div class="relative flex-1 min-w-[200px] max-w-xs">
			...
			<input
				...
				class="w-full pl-9 pr-3 py-1.5 text-sm border border-gray-200 dark:border-white/10 dark:bg-[#2c2c2e] dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-white/20 focus:border-transparent"
			/>
		</div>

		<!-- Status Filter Pills -->
		<div class="flex items-center gap-1 border-l border-gray-200 dark:border-white/10 pl-3">
			<button
				onclick={() => { statusFilter = ''; projects.loadProjects(); }}
				class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors {statusFilter === '' ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10'}"
			>
				All
			</button>
			<button
				onclick={() => { statusFilter = 'active'; projects.loadProjects('active'); }}
				class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors {statusFilter === 'active' ? 'bg-emerald-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10'}"
			>
				Active
			</button>
			<button
				onclick={() => { statusFilter = 'paused'; projects.loadProjects('paused'); }}
				class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors {statusFilter === 'paused' ? 'bg-amber-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10'}"
			>
				Paused
			</button>
			<button
				onclick={() => { statusFilter = 'completed'; projects.loadProjects('completed'); }}
				class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors {statusFilter === 'completed' ? 'bg-blue-600 text-white' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10'}"
			>
				Completed
			</button>
		</div>
		...
		<!-- Type Filter -->
		<DropdownMenu.Root>
			<DropdownMenu.Trigger class="flex items-center gap-2 px-3 py-1.5 text-xs font-medium border border-gray-200 rounded-lg bg-white hover:bg-gray-50 transition-colors {typeFilter ? 'bg-purple-50 border-purple-200' : ''}">
			...
			<DropdownMenu.Portal>
				<DropdownMenu.Content class="z-50 min-w-[160px] bg-white border border-gray-200 rounded-lg shadow-lg p-1" sideOffset={4}>
					{#each typeOptions as option}
						<DropdownMenu.Item
							class="px-3 py-2 text-xs rounded hover:bg-gray-100 cursor-pointer transition-colors {typeFilter === option.value ? 'bg-purple-50 font-medium' : ''}"
		...
		<!-- Priority Filter -->
		<DropdownMenu.Root>
			<DropdownMenu.Trigger class="flex items-center gap-2 px-3 py-1.5 text-xs font-medium border border-gray-200 rounded-lg bg-white hover:bg-gray-50 transition-colors {priorityFilter ? 'bg-orange-50 border-orange-200' : ''}">
			...
			<DropdownMenu.Portal>
				<DropdownMenu.Content class="z-50 min-w-[160px] bg-white border border-gray-200 rounded-lg shadow-lg p-1" sideOffset={4}>
					{#each priorityOptions as option}
						<DropdownMenu.Item
							class="px-3 py-2 text-xs rounded hover:bg-gray-100 cursor-pointer transition-colors {priorityFilter === option.value ? 'bg-orange-50 font-medium' : ''}"
...
{#snippet projectCard(project: Project)}
	<a href="/projects/{project.id}{embedSuffix}" class="block bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md hover:border-gray-300 hover:scale-[1.02] transition-all duration-200 cursor-pointer group">
		...
		<Dialog.Content class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg z-50 max-h-[90vh] overflow-y-auto">
			...
			<form onsubmit={handleCreateProject} class="space-y-4">
				...
				<div>
					...
					<DropdownMenu.Root>
						<DropdownMenu.Trigger class="w-full flex items-center justify-between gap-2 px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white hover:bg-gray-50 transition-colors">
							<span class={newProject.client_name ? 'text-gray-900' : 'text-gray-400'}>
						...
						<DropdownMenu.Portal>
							<DropdownMenu.Content class="z-[70] w-[var(--bits-dropdown-trigger-width)] bg-white border border-gray-200 rounded-lg shadow-lg p-1 max-h-[200px] overflow-y-auto" sideOffset={4}>
								<DropdownMenu.Item
									class="px-3 py-2 text-sm rounded hover:bg-gray-100 cursor-pointer transition-colors {!newProject.client_name ? 'bg-gray-50 font-medium' : ''}"
								...
								{#each clients as client}
									<DropdownMenu.Item
										class="px-3 py-2 text-sm rounded hover:bg-gray-100 cursor-pointer transition-colors {newProject.client_name === client.name ? 'bg-gray-50 font-medium' : ''}"
