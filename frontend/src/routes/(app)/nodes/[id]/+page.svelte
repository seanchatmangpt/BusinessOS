<script lang="ts">
	...
	// Color class maps for Tailwind JIT (static classes)
	...
	function getNodeColorClasses(color: string): { bg: string; text: string } {
		return {
			bg: bgColorClasses[color] || 'bg-gray-100',
			text: textColorClasses[color] || 'text-gray-600',
		};
	}
		...
		<div class="border-b border-gray-200 px-6 py-4 flex-shrink-0">
			...
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-4">
					{#if node}
					{@const nodeColors = getNodeColorClasses(getTypeConfig(node.type).color)}
					<div class="w-12 h-12 rounded-xl {nodeColors.bg} {nodeColors.text} flex items-center justify-center">
						...
						<div transition:slide={{ duration: 200 }}>
							...
							<div class="flex justify-end gap-2 mt-3">
								...
								<button
									onclick={savePurpose}
									disabled={isSaving}
									class="px-3 py-1.5 text-sm bg-gray-900 text-white rounded-lg hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed"
								>
					...
					<div class="bg-white border border-gray-200 rounded-xl p-5">
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
							{#each children as child}
								{@const childColors = getNodeColorClasses(getTypeConfig(child.type).color)}
								<a
									href="/nodes/{child.id}"
									class="flex items-center gap-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
								>
									<div class="w-9 h-9 rounded-lg {childColors.bg} {childColors.text} flex items-center justify-center flex-shrink-0">
										<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
