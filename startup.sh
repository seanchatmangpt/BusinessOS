...
# BusinessOS-1 Complete Startup Script
...
check_prerequisites() {
    ...
    if ! command -v psql &> /dev/null; then
        log_error "PostgreSQL not found. Install with: brew install postgresql@14"
        missing=1
    else
...
start_local() {
    ...
    if ! psql -U postgres -h localhost -c "SELECT datname FROM pg_database WHERE datname='business_os'" &>/dev/null | grep -q business_os; then
        psql -U postgres -h localhost << EOF
CREATE DATABASE business_os;
CREATE USER rhl WITH PASSWORD 'password';
ALTER ROLE rhl WITH SUPERUSER;
GRANT ALL PRIVILEGES ON DATABASE business_os TO rhl;
EOF
    ...
    # Initialize schema
    if psql -U rhl -h localhost -d business_os -c "\dt" | grep -q "user"; then
        log_warn "Schema already initialized"
    else
        psql -U rhl -h localhost -d business_os -f "$BACKEND_DIR/internal/database/init.sql"
        log_success "Schema initialized"
    fi

    # Verify connection
    if psql -U rhl -h localhost -d business_os -c "SELECT version();" &>/dev/null; then
...
start_docker() {
    ...
    if docker-compose exec -T postgres psql -U rhl -d business_os -c "SELECT version();" &>/dev/null; then
        log_success "Database connection verified"
    else
        log_error "Failed to connect to database"
        exit 1
...
EOF
    ...
    # CRITICAL: Override shell environment variables that might interfere
    # This fixes the issue where shell has DATABASE_URL set to another database (e.g., GlitchTip)
    export DATABASE_URL="postgres://rhl:password@localhost:5432/business_os?sslmode=disable"
    # Note: app:// scheme not supported by gin-cors, using http:// only
    export ALLOWED_ORIGINS="http://localhost:5173,http://localhost:5174,http://localhost:3000,http://localhost:8080,http://localhost:8001"
    # Use port 8001 since Docker uses 8000
    export SERVER_PORT="8001"
    log_info "Environment variables set for BusinessOS (using port 8001)"
...
show_status() {
    ...
    if brew services list | grep -q "postgresql@14.*started"; then
        ...
        if psql -U rhl -h localhost -d business_os -c "SELECT 1" &>/dev/null; then
    ...
    echo ""
    echo "Endpoints:"
    echo "  Database:  postgres://rhl@localhost:5432/business_os"
...
main() {
    ...
    case "$mode" in
        all)
            log_info "=== BusinessOS-1 Full Startup ==="
            check_prerequisites
            start_local
            start_backend
            start_frontend
            ...
            ;;
        docker)
            log_info "=== BusinessOS-1 Docker Startup ==="
            start_docker
            start_backend
            start_frontend
            echo ""
            ...
            ;;
        local)
            log_info "=== BusinessOS-1 Local Startup ==="
            check_prerequisites
            start_local
            echo ""
